namespace SlotMachine
{
    public static class UIMethods
    {
        ...
        public static void DisplayGameRules()
        {
            ...
            Console.WriteLine($"Each round you will be asked to bet either {Constants.BET_ONE_LINE}, {Constants.BET_TWO_LINES} up to {Constants.SLOT_MACHINE_ROWS} lines.");
            Console.WriteLine("Credit will be deducted from your balance proportionally with the number of lines you're playing, and " +
                $"will be added back into your account, in case of a win, for each winning line (e.g.match {Constants.BET_TWO_LINES} lines, win ${Constants.BET_TWO_LINES})");
        ...
        public static int SetBetAmount(GameMode chosenMode)
        {
            ...
            switch (chosenMode)
            {
                ...
                case GameMode.vertical:
                    while (!betAmountPass)
                    {
                        Console.WriteLine($"How many lines would you like to play?: ({Constants.BET_ONE_LINE} to {Constants.SLOT_MACHINE_ROWS})");
                        ConsoleKeyInfo getAnswer = Console.ReadKey();
                        betAmount = int.Parse(getAnswer.KeyChar.ToString());
                        ClearScreen();
                        if (betAmount >= Constants.BET_ONE_LINE && betAmount <= Constants.SLOT_MACHINE_ROWS)
                        {
                            betAmountPass = true;
                        }
                        else
                ...
                case GameMode.diagonal:
                    betAmount = Constants.BET_TWO_LINES;
                    break;
            }
            return betAmount;
        }
